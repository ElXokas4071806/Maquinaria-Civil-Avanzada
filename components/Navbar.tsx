      ...
      <nav style={{ background: '#000', color: '#fff', position: 'sticky', top: 0, zIndex: 50, boxShadow: '0 2px 8px rgba(0,0,0,0.3)' }}>
        <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '14px 16px', display: 'flex', alignItems: 'center', gap: '16px', justifyContent: 'space-between' }}>
          ...
          <button onClick={() => setCartOpen(true)}
            style={{ background: 'none', border: 'none', cursor: 'pointer', position: 'relative', display: 'flex', alignItems: 'center', gap: '8px', color: '#fff', flexShrink: 0, marginLeft: 'auto' }}>
        ...
        {/* Menú desplegable */}
        <div style={{ background: '#111', overflow: 'hidden', maxHeight: menuOpen ? '600px' : '0', transition: 'max-height 0.35s ease' }}>
          <div style={{ padding: '16px', display: 'flex', flexDirection: 'column', gap: '4px' }}>

            {/* Barra de búsqueda */}
            <div ref={searchRef} style={{ position: 'relative', marginBottom: '8px' }}>
              <div style={{ display: 'flex', alignItems: 'center', background: '#222', borderRadius: '999px', padding: '10px 16px', gap: '8px' }}>
                ...
                {searchQuery && (
                  <button onClick={() => { setSearchQuery(''); setSuggestions([]); setShowSuggestions(false) }}
                    style={{ background: 'none', border: 'none', color: '#9ca3af', cursor: 'pointer', fontSize: '18px', lineHeight: 1, padding: 0, flexShrink: 0 }}>
                    ×
                  </button>
                )}
              </div>
              ...
              {showSuggestions && suggestions.length > 0 && (
                <div style={{ background: '#fff', borderRadius: '12px', boxShadow: '0 8px 24px rgba(0,0,0,0.15)', marginTop: '8px', overflow: 'hidden' }}>
